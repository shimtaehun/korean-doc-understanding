...
- **결과**:
  ...
  | Metric | 값 |
  |--------|-----|
  | Best Field F1 | 0.1522 |
  | val/CER | 0.6935 |

- **분석**:
  - 실험 4(F1=0.1856) 대비 **성능 하락** → lr 낮춤 + epoch 감소로 학습량이 줄어버림
  - lr=5e-5에서 epoch=20은 수렴 부족으로 보임
  - dropout 0.1 강화가 오히려 학습을 방해했을 가능성
  - 변경사항을 한 번에 여러 개 바꿔서 원인 특정 어려움 → 다음 실험은 하나씩 분리
  - **다음 액션**: label_smoothing=0.1 추가 (seq2seq 오버피팅 억제에 더 효과적), 나머지는 실험 4 설정으로 복구
- **WandB**:
